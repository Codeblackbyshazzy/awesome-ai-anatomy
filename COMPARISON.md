...
*Last updated: 2026-04-07 • 11 projects analyzed*

---

## Quick Reference

With 11 projects, we split the table into three groups for readability.

### Group A — Agent Frameworks & Coding Agents

| | Claude Code | DeerFlow 2.0 | Hermes Agent | oh-my-claudecode | Goose | Codex CLI |
|--|------------|-------------|-------------|-----------------|-------|-----------|
| **Creator** | Anthropic | ByteDance | Nous Research | Yeachan Heo | Block Inc | OpenAI |
| **Stars** | N/A (proprietary, leaked) | 58K ⭐ | 26K ⭐ | 24K ⭐ | 37K ⭐ | 27K+ ⭐ |
| **Language** | TypeScript | Python + TypeScript | Python | TypeScript | Rust + TypeScript | Rust |
| **Lines of Code** | ~510K | — | ~260K | ~194K | ~198K | ~549K |
| **Type** | Standalone CLI | Standalone framework | Standalone CLI | Claude Code plugin | Standalone CLI + Desktop | Standalone CLI |
| **License** | Proprietary | MIT | MIT | MIT | Apache-2.0 | Apache-2.0 |
...
| Dimension | Claude Code | DeerFlow 2.0 | Hermes Agent | oh-my-claudecode | Goose | Codex CLI |
|-----------|------------|-------------|-------------|-----------------|-------|-----------|
| **Agent loop** | Monolithic `while(true)` in a single class | LangGraph + 14-layer middleware chain | Monolithic 9K-line `run_agent.py` | Plugin layer on Claude Code + team pipeline | `Agent.reply()` dispatcher loop (900-line struct) | Queue-pair request/response with Guardian AI approval gate |
| **Extensibility** | Internal tool registry via `buildTool()` | Middleware (add/remove without touching core) | Monolithic (every change touches one file) | 19 specialized agent definitions | MCP-first: 6 extension types (stdio/builtin/platform/HTTP/frontend/inline_python) | Built-in tool set (shell, file ops), no plugin system |
| **Config format** | CLI flags + `.claude/` directory | YAML (`config.yaml`) | YAML + dotfiles | Plugin settings + `.omc/` | YAML + profiles | CLI flags + config file |
| **Deployment** | Bun binary (standalone) | Nginx + LangGraph Server + Gateway API | pip install + CLI | npm package (requires Claude Code host) | Single Rust binary (CLI) or Electron (desktop) | Single Rust binary (standalone CLI) |
...
| Dimension | Claude Code | DeerFlow | Hermes | OMC | Goose | Pi Mono | Dify | Codex CLI |
|-----------|:----------:|:--------:|:------:|:---:|:-----:|:-------:|:----:|:---------:|
| **Sub-agent support** | ❌ | ✅ (thread pool, max 3) | ✅ (in-process, max 3, depth 1) | ✅ (19 role-based via tmux) | ❌ (single agent) | ❌ (single agent) | ✅ (child workflow engines) | ❌ (single agent) |
| **Multi-model** | Claude only | Single provider (configurable) | Any provider (OpenRouter) | Claude + Codex + Gemini | 30+ providers | All major providers (lazy-loaded) | All major providers | OpenAI models only |
| **IPC mechanism** | N/A | Thread pool + shared state | In-process function call | File-based inbox/outbox (mkdir locking) | N/A | N/A | GraphEngine child spawning | Queue-pair (request/response channels) |
| **Task pipeline** | None | None | None | plan → exec → verify → fix (with retry) | None | Steering + follow-up queues | Visual DAG (user-designed) | None |
| **Model routing** | Single | Config-based | Config-based | Tiered: Haiku → Sonnet → Opus | Config-based | Config-based + stealth mode | Per-node in workflow | Single (configurable) |
...
| Dimension | Claude Code | DeerFlow | Hermes | OMC | Goose | Codex CLI |
|-----------|------------|----------|--------|-----|-------|-----------|
| **Memory storage** | `.claude/` rules file (manual) | JSON file (hierarchical schema) | MEMORY.md + USER.md (markdown) | Project + user skill scopes | MCP memory extension (built-in) | None (stateless per session) |
| **Memory structure** | Flat rules/instructions | 3 time horizons (user context / history / facts) | Flat entries with § delimiter | Skill YAML files with triggers | Key-value via MCP | N/A |
| **Update mechanism** | Manual only | LLM-extracted, debounced, async | Agent tool calls (add/replace/remove) | Auto-learning via `/learner` | MCP tool calls | N/A |
| **Confidence scoring** | ❌ | ✅ (per-fact 0-1) | ❌ | ❌ | ❌ | ❌ |
| **Cross-session recall** | ❌ | ❌ | ✅ (FTS5 + LLM summarization) | ❌ | ✅ (SQLite sessions) | ❌ |
| **Prompt cache optimization** | N/A | ❌ (memory can change mid-session) | ✅ (frozen snapshot at session start) | N/A | ❌ | N/A |
| **Concurrent write safety** | N/A | ❌ (no file locking ⚠️) | ✅ (fcntl file locking) | ✅ (mkdir-based locking) | N/A | N/A |
...
| Dimension | Claude Code | DeerFlow | Hermes | OMC | Goose | Codex CLI |
|-----------|------------|----------|--------|-----|-------|-----------|
| **Loop detection** | Built-in (details unknown) | ✅ Hash-based, warn@3, kill@5 | Max iterations limit | Not documented | ✅ RepetitionInspector | ❌ None documented |
| **Cost budgets** | N/A (Anthropic-hosted) | ❌ None | ❌ None | Token tracking, no hard limit | ❌ None | ❌ None |
| **Sandbox isolation** | `sandbox-exec` (macOS), none (Linux) | Local filesystem or Docker | 6 backends (Local/Docker/SSH/Daytona/Singularity/Modal) | Inherits Claude Code sandbox | Docker + platform-extension scoping | ✅ 3-OS sandbox (macOS seatbelt, Linux landlock, Windows Job) |
| **Tool inspection** | N/A | N/A | N/A | N/A | ✅ 5-inspector pipeline (Security → Egress → Adversary → Permission → Repetition) | ✅ Guardian AI approval gate |
| **Auth/RBAC** | N/A (local CLI) | ❌ None (advisory only) | DM pairing for messaging | Inherits Claude Code | N/A (local CLI) | N/A (local CLI) |
...
| Dimension | Claude Code | DeerFlow | Hermes | OMC | Goose | Pi Mono | Codex CLI |
|-----------|------------|----------|--------|-----|-------|---------|-----------|
| **Compaction strategy** | 4-layer cascade (pinned → summarize → evict → truncate) | SummarizationMiddleware (configurable) | 5-step (prune tools → protect head/tail → structured summary → iterative update) | Inherits Claude Code | Auto-compact + tool-pair summarization | LLM-based summary, extension-overridable | Conversation history truncation |
| **Irreversible?** | ✅ (once compressed, gone) | Configurable | Iterative fallback | Inherited | Summarized (replaceable) | Summarized | ✅ (truncated) |
| **Overflow recovery** | N/A | N/A | N/A | N/A | N/A | ✅ Auto-remove error, compact, retry | N/A |

---

## Communication & Channels

| Capability | Claude Code | DeerFlow | Hermes | OMC | Goose | Pi Mono | Dify | Codex CLI |
|------------|:----------:|:--------:|:------:|:---:|:-----:|:-------:|:----:|:---------:|
| **CLI/TUI** | ✅ (Bun TUI) | ✅ | ✅ (full TUI) | ✅ (via Claude Code) | ✅ (Rust CLI) | ✅ (custom TUI library) | ❌ | ✅ (Rust TUI) |
...
Design patterns identified across all 11 projects:

| Pattern | CC | DF | HM | OMC | Goose | Pi | Dify | LP | MF | GA | CX |
|---------|:--:|:--:|:--:|:---:|:-----:|:--:|:----:|:--:|:--:|:--:|:--:|
...
| Tool inspection pipeline | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ | — | ❌ | ❌ | ✅ (Guardian) |
| Visual workflow builder | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | — | ❌ | ❌ | ❌ |
| Validator ecosystem (Hub) | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | — | ❌ | ✅ | ❌ |
| Comptime code generation | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ |
| Queue-pair architecture | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | — | ❌ | ❌ | ✅ |
| Cross-platform sandbox | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | — | ❌ | ❌ | ✅ (3-OS) |

> **Legend:** CC=Claude Code, DF=DeerFlow, HM=Hermes, OMC=oh-my-claudecode, Pi=Pi Mono, LP=Lightpanda, MF=MiroFish, GA=Guardrails AI, CX=Codex CLI. "—" means not applicable (not an agent framework).
...
| No cost budgets | DeerFlow, Hermes, Goose, MiroFish, Codex CLI | High |
...
## Key Insight: What 11 Projects Tell Us About the AI Agent Landscape

After analyzing 11 projects totaling over **3.5 million lines of code**, several macro-trends emerge:
...
Goose is the only project with a proper tool inspection pipeline (5 inspectors including LLM-based adversarial review). Codex CLI takes a different approach with its Guardian AI approval gate for command execution, which is solid but narrower in scope. Everyone else bolts on permission checks or skips them entirely. Dify gets credit for its SSRF proxy and sandbox isolation. Codex CLI stands out for cross-platform sandbox support (macOS seatbelt, Linux landlock, Windows Job objects) — most projects only support one OS. Nobody has implemented cost budgets that actually enforce spending limits — this is the single biggest gap across all 11 projects.
...
The 11 projects span four distinct categories:
- **Coding agents** (Claude Code, Hermes, OMC, Pi Mono, Goose, Codex CLI) — interactive terminal tools
...
After analyzing 11 projects totaling ~3.5M lines of code:
...
- **Security:** Take Goose's 5-inspector tool inspection pipeline. Add Dify's SSRF proxy and sandbox isolation. Take Codex CLI's cross-platform sandbox approach (seatbelt/landlock/Job objects) for OS-level isolation. **Actually implement cost budgets** — nobody has this yet.
